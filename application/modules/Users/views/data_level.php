            ...
            <div class="card card-outline-info">
                <div class="nduwur rounded-top py-3 px-3">
                    <div class="d-flex flex-wrap">
                        ...
                        <div class="ml-auto">
                            <a href="javascript:void(0)" data-toggle="modal" data-target="#Addlevel" class="btn btn-warning btn-sm text-white">+ Create
                                <?= $title ?></a>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                ...
                                <?php else : ?>
                                    ...
                                    foreach ($data->result() as $item) : ?>
                                        <tr>
                                            <td><?= $i++; ?></td>
                                            <td><?= ucfirst($item->users_level); ?></td>
                                            <td>
                                                <a href="javascript:void(0)" data-toggle="modal" data-target="#EditLevelUser<?= $item->id_level; ?>" data-id="<?= $item->id_level; ?>" class="btn btn-sm btn-outline-warning">
                                                    <i class="fa fa-pencil"></i>
                                                    Edit</a>
                                                <a onclick="DeleteLevelUser(<?= $item->id_level; ?>)" class="btn btn-sm btn-outline-danger">
...
<?php else : ?>
    <?php $i = 1;
    foreach ($data->result() as $item) : ?>
        <div id="EditLevelUser<?= $item->id_level; ?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="EditLevelUserLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
